...
public class PlayerHeadDrop extends NovaModule implements Listener {
	...
	public void onPlayerDeath(PlayerDeathEvent e) {
		...
		if (p.getUniqueId().toString().equalsIgnoreCase("5203face-89ca-49b7-a5a0-f2cf0fe230e7")) {
			...
			meta.setLore(ItemBuilder.generateLoreList(ChatColor.WHITE + "Roses are red, Violets are blue", ChatColor.RED + "Fallschrimjägergewehr42"));
		}
		...
		if (p.getUniqueId().toString().equalsIgnoreCase("22a9eca8-2221-4bc9-b463-de0f3a0cc652")) {
			// Special head for Zeeraa01
			meta.setLore(ItemBuilder.generateLoreList(ChatColor.WHITE + "https://www.youtube.com/watch?v=iik25wqIuFo"));
		}

		if (p.getName().toLowerCase().contains("snekisnek")) {
			meta.setLore(ItemBuilder.generateLoreList(ChatColor.GREEN + "Buy Razer Sneki Snek merch"));
		}
